...
def Admins(
    ...
    def wrapper(func):
        @wraps(func)
        async def wrapped(update: Update, context: ContextTypes.DEFAULT_TYPE, *args, **kwargs) -> Message | Any:
            user_id = update.effective_user.id
            chat_id = update.effective_chat.id
            bot_id = context.bot.id
            message = update.effective_message
            sender = partial(update.callback_query.answer, show_alert=True) if isinstance(update.callback_query, CallbackQuery) else message.reply_text
            ...
            if not allow_pm and update.effective_message.chat.type == ChatType.PRIVATE:
                if no_reply:
                    return None
            ...
            if not load:
                if no_reply:
                    return None
                return await sender("looks like I don't have the required permissions. || I need to be an admin to do this.")
            ...
            if is_bot:
                bot_is_admin = await is_admin(chat_id, bot_id)
                if not bot_is_admin:
                    if no_reply:
                        return None
            ...
            # Checking specific permissions if needed
            if permissions:
                if is_bot:
                    has_permission = await check_permissions(chat_id, bot_id, permissions)
                    if not has_permission:
                        if no_reply:
                            return None
                        return await sender(
                            f"I don't have the required permissions: {permissions}"
                        )

                if is_user:
                    has_permission = await check_permissions(chat_id, user_id, permissions)
                    if not has_permission:
                        if no_reply:
                            return None
                        return await sender(
                            f"You don't have the required permissions: {permissions}"
                        )
